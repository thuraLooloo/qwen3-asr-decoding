...
def apply_audio_perturbation(
    ...
    *,
    ...
    """
    Apply one acoustically-motivated negative perturbation to a waveform, for use as a
    negative view in contrastive decoding (Whisper-CD, arXiv:2603.06193).

    The perturbed waveform always has the same length as `wav`, so it produces the same
    number of audio placeholder tokens and can be batched alongside the clean audio.

    Args:
        kind: One of "noise" (additive Gaussian noise at a target SNR), "silence"
            (all-zero waveform), or "shift" (temporal left-shift with zero-padding).
        wav: Mono float32 waveform.
        snr_db: Target signal-to-noise ratio in dB, used when kind == "noise".
        shift_sec: Left-shift amount in seconds, used when kind == "shift".

    Returns:
        np.ndarray: Perturbed waveform with the same shape as `wav`.
    ...
    wav = np.asarray(wav, dtype=np.float32)
    if kind == "noise":
        return _perturb_gaussian_noise(wav, snr_db=snr_db)
    if kind == "silence":
        return np.zeros_like(wav)
    if kind == "shift":
        return _perturb_temporal_shift(wav, shift_sec=shift_sec)
    raise ValueError(f"Unknown perturbation kind: {kind!r}. Expected one of: 'noise', 'silence', 'shift'.")


def _perturb_gaussian_noise(wav: np.ndarray, snr_db: float) -> np.ndarray:
    if wav.size == 0:
        return wav.copy()
    signal_power = float(np.mean(np.square(wav, dtype=np.float64)))
    if signal_power <= 0.0:
        return wav.copy()
...
def _perturb_temporal_shift(wav: np.ndarray, shift_sec: float) -> np.ndarray:
    shift = int(round(shift_sec * SAMPLE_RATE))
    if shift <= 0 or wav.size == 0:
        return wav.copy()
    shift = min(shift, wav.shape[0])
    return np.concatenate([wav[shift:], np.zeros((shift,), dtype=np.float32)]).astype(np.float32)
